...
exports.simplifyNode = function simplifyNode(node, dropTheName) {
  var output = {}
      , keys = Object.keys(node)
      , dropName = Boolean(dropTheName)
      ;
...
exports.moosh = function moosh(item1, item2) {
  if (item1 === undefined) {
...
exports.objectifyMarkup = function objectifyMarkup(node, dontSimplify) {
  var key
    , output = {}
    ;
  ...
  if (node.$markup) {
    node.$markup.forEach(function eachMarkupItem(item) {
      if (typeof item === 'string') {
        ...
        if (!dontSimplify) {
          output[key] = exports.moosh(output[key], simplifyAll(item, true));
        } else {
